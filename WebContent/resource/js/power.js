...
function getPageData(){
	$('#mainTable').datagrid({
		url : getRootPath()+'/power/showAllPower.do',
		pagination:true,
		columns : [ [ {
			field : 'power_id',
			title : '权限编号',
...
function mainBut_add(){
	...
	$('#fm').form('clear');
	url = getRootPath()+'/power/addOnePower.do';
}
...
function mainBut_edit(){
	...
	if (row){
		...
		$('#fm').form('load',row);
		url = getRootPath()+'/power/UpdateForPower.do';
	}else{
		alert("请选择一行操作");
	}
}
...
function mainBut_delete(){
	...
	if (row){
		...
		$.messager.confirm('确定删除','您确定要删除这个用户吗?',function(r){
			if (r){
				$.post(getRootPath()+'/power/deleteForPower.do',{id:row.power_id},function(result){
					if (result == 0){
						$.messager.show({
							title:'提示消息',
							msg:'恭喜您删除成功',
